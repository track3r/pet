...
glm::vec3 GetPositionOnCubic(const glm::vec3& startPos, const glm::vec3& startVel, const glm::vec3& endPos, const glm::vec3& endVel, float time)
{
    glm::mat4 m;
    m[0] = startPos;
    m.SetRow1(endPos);
    ...
    glm::vec3 timeVector(time * time * time, time * time, time, 1.f);
